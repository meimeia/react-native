...
export default new class {
    ...
    selectAll = action(async () => {
        this.isSelectAll.set(!this.isSelectAll.get());

        console.log( this.isSelectAll,22)
        console.log( this.isSelectAll.get(),23)
        let ids = [];
        await this.data.list.map(item => {
            item.select_status = this.isSelectAll.get() ? 1 : 0;
            ids.push(item.id);
            return item;
        });
        ids = ids.join(',');
