...
export default class submit extends Component {
    ...
    switchOnChange=()=>{
        ...
        console.log(!this.state.balanceYn)
    this.setState({balanceYn:!this.state.balanceYn})
    }
    render() {
        return (
            <View style={styles.container}>
                ...
                <TouchableWithoutFeedback onPress={() => this.selectAddress()}>
                    <View style={styles.address}>
                        <Image style={styles.addressIcon}
                               source={{uri: require('../../static/img/icon-address')}}>

                        </Image>
                        {!this.state.address.name &&
                        <Text allowFontScaling={false} style={styles.addressHint}>选择地址</Text>}
                        {this.state.address.name &&
                        ...
                        <Image style={styles.addressIconArrow}
                               source={{uri: require('../../static/img/icon-arrow')}}>
                        </Image>
                    </View>
                </TouchableWithoutFeedback>
                {/*选择地址下面的一条线*/}
                <View style={{marginBottom: px(20), flexDirection: 'row', width: px(254 * 3)}}>
                    {[...Array(3)].map((i, idx) =>
                        <Image key={idx}
                               source={{uri: require('../../static/img/bg-address-line')}}
