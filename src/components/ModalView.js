                    ...
                    <View style={DialogStyle.alert_box}>
                        ...
                        <View style={DialogStyle.alert_foot}>
                            {opt.btns.map((btn, index) => <TouchableOpacity key={index} onPress={() => {
                                    ...
                                    style={[DialogStyle.alert_foot_btn, {borderLeftWidth: index > 0 ? px(1) : 0}]}><Text
                                    style={{color: btn.color}}>{btn.txt}</Text></View></TouchableOpacity>
                            )}
                        </View>
                    </View>
                </View>
...
exports.ImageModal = class extends React.Component {
    ...
    render() {
        console.log(this.props.list,800);
        return (
            <Modal visible={this.state.showImg}
                   animationType="none"
                   transparent={true}>
                <View style={{flex:1,width: deviceWidth,height:deviceHeight,backgroundColor:"black"}}>
                    <ScrollView
                                horizontal={true}>
                        {this.props.list.map((item, index) => <TouchableWithoutFeedback key={index}
                                                                                        onPress={() => this.close()} onLongPress={()=>this.setState({showSave:true})}>
                                <View>
<Image source={{uri:item.image}} resizeMode={"contain"} style={{width:deviceWidth,height:deviceHeight}}/>
                                </View>
                            </TouchableWithoutFeedback>
                        )}
                    </ScrollView>
                    {this.state.showSave&&<View style={{backgroundColor:"#fff",position:"absolute",bottom:0}}>
                        <TouchableWithoutFeedback onPress={()=>this.saveToPhoto()}>
                            <Text style={{width:deviceWidth,height:50,textAlign: "center",lineHeight: 50}}>保存到相册</Text>
                        </TouchableWithoutFeedback>
                        <TouchableWithoutFeedback onPress={()=>this.setState({showSave:false})}>
                            <Text style={{width:deviceWidth,height:50,textAlign: "center",lineHeight: 50}}>取消</Text>
                        </TouchableWithoutFeedback>
                    </View>}
                </View>
            </Modal>
